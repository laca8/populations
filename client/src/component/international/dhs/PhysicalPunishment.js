...
const PhysicalPunishment = () => {
  const internationalDhsPhysicalReducer = useSelector(
    (state) => state.internationalDhsPhysicalReducer
  );
  const { data, error, loading } = internationalDhsPhysicalReducer;

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(listPhysical());
  }, []);
  ...
  const addInternationalDhsPhysicalReducer = useSelector(
    (state) => state.addInternationalDhsPhysicalReducer
  );
  const {
    loading: loadingAdd,
    error: errorAdd,
...
export default PhysicalPunishment;
