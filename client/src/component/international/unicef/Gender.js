...
const Gender = () => {
  ...
  const indicators = [
    ...
    "Proportion of adolescents and adults (aged 15 years and older) with an account at a financial institution or mobile money service provider",
    "Proportion of individuals who own a mobile telephone",
    "Labour force participation rate",
    "Paternity leave benefits",
    "Labour force unemployment rate",
  ];
  const unicefGenderReducer = useSelector((state) => state.unicefGenderReducer);
  const { data, error, loading } = unicefGenderReducer;

  const dispatch = useDispatch();
  useEffect(() => {
    dispatch(listUnicefGender(indicator));
  }, [indicator]);
  ...
  const addUnicefGenderReducer = useSelector(
    (state) => state.addUnicefGenderReducer
  );
  const {
    loading: loadingAdd,
    error: errorAdd,
